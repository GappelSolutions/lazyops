    ...
    if show_search {
        ...
        match app.pipeline_drill_down {
            crate::app::PipelineDrillDown::Tasks => {
                draw_timeline_tasks(f, app, chunks[1], border_color, focused)
            }
            crate::app::PipelineDrillDown::Runs => {
                draw_pipeline_runs(f, app, chunks[1], border_color, focused)
            }
            crate::app::PipelineDrillDown::None => {
                draw_pipeline_list(f, app, chunks[1], border_color, focused)
            }
        }
    } else {
        ...
        match app.pipeline_drill_down {
            crate::app::PipelineDrillDown::Tasks => {
                draw_timeline_tasks(f, app, area, border_color, focused)
            }
            crate::app::PipelineDrillDown::Runs => {
                draw_pipeline_runs(f, app, area, border_color, focused)
            }
            crate::app::PipelineDrillDown::None => {
                draw_pipeline_list(f, app, area, border_color, focused)
            }
        }
    }
}
...
fn draw_pipeline_list(
    ...
    let mut filtered_data: Vec<(usize, i32, String, Option<String>, bool)> =
        if search_query.is_empty() {
            ...
            app.pipelines
                ...
                .filter(|(_, p)| {
                    app.fuzzy_matcher
                        .fuzzy_match(&p.name, &search_query)
                        .is_some()
                })
                .map(|(i, p)| {
                    (
    ...
    filtered_data.sort_by(|a, b| match (a.4, b.4) {
    ...
    let display_idx = filtered_data
        .iter()
        .position(|(orig_idx, _, _, _, _)| *orig_idx == selected_idx);
    ...
    if filtered_data.is_empty() {
        ...
        let items: Vec<ListItem> = filtered_data
            .iter()
            .map(|(orig_idx, _id, name, queue_status, is_pinned)| {
...
fn draw_pipeline_runs(
    ...
    let filtered_data: Vec<(
        ...
        app.pipeline_runs
            ...
            .map(|(i, r)| {
                (
                    i,
                    r.build_number.clone(),
                    r.source_branch.clone(),
                    r.status.clone(),
                    r.result.clone(),
                )
            })
    ...
    let action_hints = app
        .pipeline_runs
        .get(selected_idx)
        .map(|run| match run.status.as_deref() {
    ...
    if filtered_data.is_empty() {
        ...
        let items: Vec<ListItem> = filtered_data
            .iter()
            .map(|(orig_idx, build_number, source_branch, status, result)| {
                let selected = *orig_idx == selected_idx;
                let prefix = if selected && focused { "▸ " } else { "  " };
...
fn draw_timeline_tasks(
    ...
    let all_task_items: Vec<TaskItem> = tasks
        .iter()
        .enumerate()
        .map(|(idx, task)| {
            let name = task
    ...
    if task_items.is_empty() {
        ...
        let items: Vec<ListItem> = task_items
            .iter()
            .map(|(orig_idx, name, state, result, _has_log)| {
        ...
        // Find display index for selected item
        let display_idx = task_items
            .iter()
            .position(|(orig_idx, _, _, _, _)| *orig_idx == selected_task_idx);
