...
fn draw_release_definitions(
    ...
    let mut filtered_data: Vec<(usize, i32, String, bool)> =
        if search_query.is_empty() || !is_releases_focused {
            app.releases
                .iter()
                .enumerate()
    ...
    filtered_data.sort_by(|a, b| match (a.3, b.3) {
    ...
    if filtered_data.is_empty() {
        ...
        let items: Vec<ListItem> = filtered_data
            .iter()
            .map(|(orig_idx, _id, name, is_pinned)| {
...
fn draw_release_list(f: &mut Frame, app: &mut App, area: Rect, border_color: Color, focused: bool) {
    let release_def_name = app
        .releases
        .get(app.selected_release_idx)
        .map(|r| r.name.as_str())
        .unwrap_or("Release");
    ...
    let action_hints = app
        ...
        .map(|release| {
            let has_active = release
                ...
                .map(|envs| {
                    envs.iter()
                        .any(|e| e.status.as_deref() == Some("inProgress"))
                })
                .unwrap_or(false);
    ...
    if app.release_list.is_empty() {
        ...
        app.release_item_list_state
            .select(Some(app.selected_release_item_idx));
        let list = List::new(items).block(block);
        f.render_stateful_widget(list, area, &mut app.release_item_list_state);
    }
}

fn draw_release_stages(
    f: &mut Frame,
    app: &mut App,
    area: Rect,
    border_color: Color,
        ...
        .map(|stage| {
            ...
            if has_pending_approval {
                ...
                match stage.status.as_deref() {
                    ...
                    Some("succeeded") | Some("failed") | Some("canceled") | Some("rejected")
                    | Some("notStarted") => " [T]rigger",
                    _ => "",
                }
            }
        })
    ...
    if app.release_stages.is_empty() {
        ...
        app.release_stage_list_state
            .select(Some(app.selected_release_stage_idx));
        let list = List::new(items).block(block);
        f.render_stateful_widget(list, area, &mut app.release_stage_list_state);
    }
}

fn draw_release_tasks(
    f: &mut Frame,
    app: &mut App,
    area: Rect,
    border_color: Color,
    ...
    if app.release_tasks.is_empty() {
        ...
        app.release_task_list_state
            .select(Some(app.selected_release_task_idx));
        let list = List::new(items).block(block);
